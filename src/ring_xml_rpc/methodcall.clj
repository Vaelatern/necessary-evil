...
(defn parse-struct-member [m] (let [name (xml1-> m :name first-child)
                                  ...
                                  (if (or name val)
                                      [(keyword (-> name text .trim)), (parse-value val)]
                                      [nil, nil])))
...
(defn- elem 
    "elem is a utility function to make creating xml/element structs (minus attributes)"
    ([name] (struct xml/element name))
    ([name children] (struct xml/element name nil children)))

(defprotocol ValueTypeElem
    "ValueTypeElem defines a single function 'value-type-elem' that is used to emit various types
     as values for the xml-rpc format"
    (value-type-elem [x]))
...
(extend-protocol ValueTypeElem
    ...
    org.joda.time.DateTime
        (value-type-elem [this] (elem :dateTime.iso8601 [(time-format/unparse winer-time this)]))
...
(defn emit-method-call
    ...
    (let [name-elem   (elem :methodName [(-> mc :method-name name)])
          ...
          children (if (seq params) 
                         [name-elem (elem :params (map #(elem :param [(value-elem %)]) params))]
                         [name-elem])]
